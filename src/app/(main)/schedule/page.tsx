...
import { generateNotificationMessage, type GenerateNotificationInput } from '@/ai/flows/generate-notification-flow'; // Import AI notification generator
  ...
  const handleAddItem = async <FormValues extends Record<string, any>>(
    ...
    try {
      ...
      if (collectionName === SCHEDULE_COLLECTION) {
        ...
        aiNotificationInput.itemName = data.subject; // Ensure itemName is subject for homework
        aiNotificationInput.itemDetails = `Task: ${data.task}, Due: ${data.dueDate}`;
      } else if (collectionName === UPDATES_COLLECTION) {
        aiNotificationInput.itemDetails = data.message.substring(0, 50) + (data.message.length > 50 ? '...' : ''); // Snippet
      }
      ...
      try {
        ...
        console.error(`SchedulePage: Error adding AI-generated notification for ${notificationType}:`, notifError);
        // Fallback to simple notification if AI fails
        await addDoc(collection(db, NOTIFICATIONS_COLLECTION), {
          message: `${t(successMessageKey)}: ${aiNotificationInput.itemName}`,
          type: notificationType,
          link: '/schedule',
          timestamp: serverTimestamp(),
          autoGenerated: true
