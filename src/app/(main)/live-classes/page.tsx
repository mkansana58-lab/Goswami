...
export default function LiveClassesPage() {
  ...
  const fetchLiveClasses = async () => {
    ...
    console.log("Attempting to fetch live classes from Firestore...");
    try {
      ...
      querySnapshot.forEach((doc) => {
        const data = doc.data();
        console.log("Raw data for class", doc.id, data);
        fetchedClasses.push({
          id: doc.id,
          title: data.title,
      ...
      console.log("Live classes fetched successfully:", fetchedClasses.length, fetchedClasses);
    } catch (error: any) {
      ...
      console.log("Finished fetching live classes attempt.");
    }
  };
  ...
  const onSubmit: SubmitHandler<LiveClassFormValues> = async (data) => {
    ...
    try {
      ...
      console.log("New class payload for Firestore:", newClassPayload);
      ...
      try {
        const notificationPayload = {
          ...
          type: 'new_live_class', // Consistent type for easier icon mapping
          link: '/live-classes',
          timestamp: serverTimestamp(),
          autoGenerated: true
        };
        console.log("Attempting to add notification to Firestore:", notificationPayload);
        await addDoc(collection(db, "notifications"), notificationPayload);
        console.log("Notification for new live class added successfully.");
      } catch (notifError: any) {
        console.error("Error adding notification for live class:", { message: notifError.message, code: notifError.code, stack: notifError.stack, fullError: notifError });
